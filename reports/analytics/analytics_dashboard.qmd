...
data <- logs %>%
  ...
  # When there are two pins, we separaate them.
...
Extract year refers to the year the property was re-assessed. This can be the same as the query year, but does not have to be.
...
data %>%
  group_by(cluster_id) %>%
  summarise(
    # We assume if that any request in the cluster is eligible, the whole
    # cluster is eligible. It also removes NA values.
    eligibility = if (any(is_report_eligible, na.rm = TRUE)) {
      "Eligible"
    } else {
      "Not Eligible"
    },
...
data %>%
  ...
  count(browser, name = "request_count", sort = TRUE) %>%
  plot_ly(
