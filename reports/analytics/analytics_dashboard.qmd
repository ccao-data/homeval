...
walk(dates_to_fetch, function(d) {
  ...
  if (d == today(tzone = tz_local) & refresh_today == FALSE &
    fs::file_exists(out_file)) { # no lintr
    message("Skipping ", format(d), " (today's data, not refreshing).")
    return(invisible(NULL))
  }
