...
data %>%
  ...
  mutate(
    reason_clean = case_when(
      is.na(reason_report_ineligible) & is.na(pin) ~ "Unmatched PIN",
      is.na(reason_report_ineligible) ~ "Un-joined PIN",
...
data %>%
  filter(
    uri_indicator == "Query",
    # keep ineligible or null eligible cases.
    # This allows us to find unjoined pins
    is_report_eligible %in% c(FALSE, NA),
    !is.na(pin) # Removes queries which do not have a PIN associated with it
  ) %>%
  mutate(
    reason_clean = case_when(
      is.na(reason_report_ineligible) ~ "Un-joined PIN",
