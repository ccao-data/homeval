...
data <- logs %>%
  ...
  # There are duplicate messages. This keeps one per query.
  ...
  mutate(
    ...
    # This is an indiicator for the main query, or the query
...
```{r}
# We want to keep multi-cards just for the class related
data_class <- data %>%
...
```{r}
# This removes the duplicate cards from the previous query
data <- data %>%
  distinct(message, .keep_all = TRUE) %>%
