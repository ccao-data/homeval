...
data <- logs %>%
  ...
  left_join(
    assessment_card %>%
      ...
      select(meta_pin, char_class, assessment_year, meta_triad_name, meta_card_num, is_report_eligible, reason_report_ineligible, meta_township_code),
    by = c("pin" = "meta_pin")
  )
```
...
```{r}
data %>%
  mutate(char_class = ifelse(is.na(char_class), "NA", as.character(char_class))) %>%
  group_by(char_class) %>%
...
# Request count by class over time (total)
...
data %>%
  ...
  layout(
    barmode = "stack",
    title = "Request Count by Class Over Time",
...
```
## Request count by extract year over time (total)
...
```
## Request count by extract year over time (%)
...
data %>%
  ...
  layout(
    barmode = "stack",
    title = "Request Count by Reassessment Year Over Time (%)",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Percentage", ticksuffix = "%"),
    legend = list(title = list(text = "Year"))
  )
...
data %>%
  ...
  layout(
    title = "Request Count by Report Eligibility",
    xaxis = list(title = "Eligibility"),
    yaxis = list(title = "Count")
  )
```
## Request count by report eligibility reason
...
data %>%
  ...
  layout(
    title = "Request Count by Report Eligibility Reason",
...
data %>%
  ...
  plot_ly(
    ...
    type  = "bar",
    hovertemplate = "Date: %{x}<br>Reason: %{fullData.name}<br>Count: %{y}<extra></extra>"
  ) %>%
  layout(
    barmode = "stack",
    title = "Request Count by Report Eligibility Reason Over Time (Total)",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Count"),
...
data %>%
  ...
  plot_ly(
    ...
    color = ~reason_report_ineligible,
    type  = "bar",
    hovertemplate = "Date: %{x}<br>Reason: %{fullData.name}<br>Share: %{y:.1f}%<extra></extra>"
  ) %>%
  layout(
    barmode = "stack",
    title = "Request Count by Report Eligibility Reason Over Time (%)",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Percentage", ticksuffix = "%"),
    legend = list(title = list(text = "Eligibility Reason"))
  )
...
## Request count by township and reassessment year {.tabset}
### 2024
```{r}
...
data %>%
  ...
  plot_ly(
    ...
    type = "scatter",
    mode = "lines+markers",
    hovertemplate = "Date: %{x}<br>Status: %{fullData.name}<br>Count: %{y}<extra></extra>"
  ) %>%
  layout(
    title = "Distribution of HTTP Response Status Codes Over Time",
    xaxis = list(title = "Date"),
    yaxis = list(title = "Count"),
...
data %>%
  mutate(
    ua_raw = `cs(User-Agent)`,
    ua     = str_to_lower(ua_raw),
    browser = case_when(
      str_detect(ua, "bot|crawler|spider|slurp|baiduspider|ahrefs|semrush|facebookexternalhit|bingpreview") ~ "Bot / Crawler",
      str_detect(ua, "edg/|edge/") ~ "Microsoft Edge",
      str_detect(ua, "opr/|opera") ~ "Opera",
      str_detect(ua, "samsungbrowser") ~ "Samsung Internet",
      str_detect(ua, "brave") ~ "Brave",
      str_detect(ua, "safari") & !str_detect(ua, "chrome|crios|chromium|opr/|edg/|edge/") ~ "Safari",
      str_detect(ua, "firefox|fxios") ~ "Firefox",
      str_detect(ua, "msie|trident/") ~ "Internet Explorer",
      str_detect(ua, "chrome|crios|chromium") ~ "Chrome",
      TRUE ~ "Other"
    )
  ) %>%
  group_by(browser) %>%
...
data %>%
  mutate(
    ...
    is_hit = if_else(
      str_detect(coalesce(result_type, ""), regex("hit|redirect", ignore_case = TRUE)),
      TRUE, FALSE
    )
  ) %>%
  group_by(date) %>%
