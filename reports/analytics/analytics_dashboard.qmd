...
params:
  ...
  # Lists of IP's or excluded dates are separated by '-'.
...
dates_to_fetch <- c(
  setdiff(
    all_dates[
      (all_dates != end_date) |
        (is.null(params$query_today) || isTRUE(params$query_today))
    ],
    c(
      cached_dates,
  ...
  if (is.null(params$query_today) || isTRUE(params$query_today)) end_date else NULL
) |> sort()
...
data <- logs %>%
  ...
  filter(
    # Logs are created and cached every time a request is made.
    # This means that if cached files used a different query,
    # there will be more cached files that are unnecessary.
    # Because of this, we filter based on the params a second time.
    if (!is.null(params$end_date) && nzchar(params$end_date))
      date <= as.Date(params$end_date)
    else TRUE,


    if (!is.null(params$start_date) && nzchar(params$start_date))
      date >= as.Date(params$start_date)
    else TRUE,

    # excluded dates (list of strings)
    if (!is.null(params$excluded_dates) && length(params$excluded_dates) > 0)
      !(date %in% as.Date(params$excluded_dates))
    else TRUE,

    # excluded IPs
    if (!is.null(params$ip_addresses_to_filter) && length(params$ip_addresses_to_filter) > 0)
