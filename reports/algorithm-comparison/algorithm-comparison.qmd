...
format_agg_stats <- function(df,
                             table_type = c("overall", "township", "class")) {
  table_type <- match.arg(table_type)

  freeze_cols_lookup <- list(
    overall  = c("Overall", "Algorithm", "description"),
    township = c("Triad", "Township", "Algorithm", "description"),
    class    = c("Class", "Algorithm", "description")
  )

  freeze_cols <- freeze_cols_lookup[[table_type]]
  freeze_cols <- freeze_cols[freeze_cols %in% colnames(df)]

  freeze_idx <- match(freeze_cols, colnames(df))
  freeze_idx <- freeze_idx[!is.na(freeze_idx)]
  ...
  dat <- df %>%
    datatable(
      ...
      options = list(
        ...
        scrollX = TRUE,
        fixedColumns = list(
          leftColumns = length(freeze_idx)
        )
      )
    ) %>%
    formatRound(
      c("Med. Comp. Score", "Corr. Comp. PIN Pred.", "Corr. Comp. Target Sale"),
      digits = 2
    ) %>%
    formatRound(
      c(
        "Num. Targets", "Num. Sales", "Med. Comp. Dist. (ft)",
        "Avg. Comp. Dist. (ft)"
      ),
      digits = 0
    ) %>%
